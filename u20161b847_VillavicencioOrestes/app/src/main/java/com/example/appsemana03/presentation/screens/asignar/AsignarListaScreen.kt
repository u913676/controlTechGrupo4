...
// Lista de equipos del módulo Asignar. Solo los equipos "Disponible" pasan al formulario.
@Composable
fun AsignarListaScreen(container: AppContainer, onIrAlFormulario: () -> Unit) {
    ...
    Column(
        ...
        uiState.equipos.forEach { equipo ->
            EquipoCard(
                equipo = equipo,
                onClick = {
                    viewModel.intentarAsignar(equipo) {
                        onIrAlFormulario()
                    }
                }
            )
